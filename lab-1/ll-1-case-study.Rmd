...
that *it uses the entire data set* `train_test_split` data---not just
the training data.
...
```{r}
final_fit %>% 
    collect_predictions() %>% # see test set predictions b/c we can't read final_fit directly
...
final_fit %>% 
    ...
    select(.pred_class, code) %>% # just to make the output easier to view 
    mutate(correct = .pred_class == code) %>% # create a new variable, correct, telling us when the model was and was not correct
    tabyl(correct) #tabulate the frequency of correct predictions
```
...
-   The ML trained model predicted the correct code with 87% accuracy.
...
-   An ROC AUC value is 0.94 which means this is close to a perfect
    prediction model.
